...
const addProductButton =
	"<button onclick='addProduct()' class='btn btn-outline-secondary add-btn'><i class='far fa-plus-square'></i></button>";
...
async function getProducts() {
	...
	const response = await fetch(
		`/manage-catalogs/products/${catalogId}?get-products=true`,
	);
...
async function handleCreate() {
	...
	fetch(`/manage-catalogs/products/${catalogId}`, {
		...
		.then((res) => res.json())
		.then((product) => {
			if (product.message === "failed")
				return Swal.fire("Alert", "Something went wrong", "warning");
			allProducts += ProductCard(catalogId, product);
			updateProductsContainer();
		})
...
async function addProduct() {
	...
	await Swal.mixin(MODAL_CONFIG).fire({
		title: "Add product",
		html: form,
...
async function deleteProduct(id) {
	...
	const response = await fetch(
		`/manage-catalogs/products/${catalogId}?product-id=${id}`,
		{
			headers: {
				"X-CSRFToken": csrftoken,
			},
...
async function updateProduct(id, name, price, description, image) {
	...
	Swal.mixin(MODAL_CONFIG).fire({
		...
		focusConfirm: false,
		preConfirm: () => {
			const updateForm = Swal.getPopup().querySelector(
				"#update-product-form",
			);
			...
			fetch(`/manage-catalogs/products/${catalogId}?method=PUT`, {
				...
				.then((product) => {
					...
					updatedProductElement.innerHTML = ProductCardInner(
						catalogId,
						product,
					);
				})
